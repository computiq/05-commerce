...
def view_cart(request):
    cart_items = Item.objects.filter(user=request.auth.get('user'), ordered=False)
...
def add_update_cart(request, item_in: ItemCreate):
    try:
        item = Item.objects.get(product_id=item_in.product_id, user=request.auth.get('user'))
        item.item_qty += 1
        item.save()
    except Item.DoesNotExist:
        Item.objects.create(**item_in.dict(), user=request.auth.get('user'))
...
def reduce_item_quantity(request, id: UUID4):
    item = get_object_or_404(Item, id=id, user=request.auth.get('user'))
    if item.item_qty <= 1:
        item.delete()
...
def create_order(request):
    ...
    * calculate the total
    ...
    user_items = Item.objects.filter(user=request.auth.get('user')).filter(ordered=False)
